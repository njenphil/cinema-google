import React from "react"

const Search = () => {
  return (
    <section className="search | section-padding">
      <div className="container">
        <form className="search-form">
          <div className="input-wrapper">
            <input type="text" placeholder="Search for a show" />
            <div className="search-icons">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="white" className="size-6">
                <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
              </svg>

              <i className="fas fa-times"></i>
            </div>
          </div>
        </form>
      </div>
      <div className="search-results" style={{ display: "none" }}>
        <div className="container">
          <div className="spinner-wrapper hide-spinner">
            <span className="loader"></span>
          </div>
          <div className="actual-results">
            <div className="no-results" style={{ display: "none" }}>
              <div className="no-results-content">
                <i className="far fa-frown"></i>
                <p className="no-result-heading">No Results Matching your Search</p>
                <p>Sorry, but nothing matched your search terms. Please try again with some different keywords.</p>
              </div>
            </div>
          </div>
          <div className="results-found" style={{ display: "none" }}></div>
        </div>
      </div>
    </section>
  )
}

export default Search
